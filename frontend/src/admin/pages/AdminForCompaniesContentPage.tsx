...
import AdminSiteContentForCompaniesEditorSection from "../components/admin_site_content/AdminSiteContentForCompaniesEditorSection";
...
function AdminForCompaniesContentPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setContent(data as SiteContent);
      setSaveMessage("For Companies content saved successfully.");
    } catch (error) {
      setErrorMessage(
    ...
    return (
      <section className="admin-site-content-page-state">
        <div className="container">
          <h1>Loading For Companies content...</h1>
        </div>
      </section>
    );
  }
  ...
  if (errorMessage && !content) {
    return (
      <section className="admin-site-content-page-state">
        <div className="container">
          <h1>Could not load For Companies content</h1>
          <p>{errorMessage}</p>
        </div>
      </section>
    );
  ...
  return (
    <>
      ...
      <div className="admin-site-content-layout">
        <div className="container">
          {saveMessage ? (
            <p className="admin-site-content-page__save-message">
              {saveMessage}
            </p>
          ) : null}

          {errorMessage ? (
            <p className="admin-site-content-page__error-message">
              {errorMessage}
            </p>
          ) : null}

          <AdminSiteContentForCompaniesEditorSection
            content={content}
...
export default AdminForCompaniesContentPage;